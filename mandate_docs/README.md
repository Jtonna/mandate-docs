...
```json
{
  ...
  "code": {
    ...
    "JTBXrGc3": "tests/common/mod.rs",
    "r7yc78R9": "tests/validation_fixtures.rs"
  },
...
build.rs                      generates the fixture case module list
                              at compile time
docs/
  architecture/
...
| `validation_fixtures.rs` | The root of the fixture cases; the module list is generated by `build.rs` from the case directories. |
...
several things.
A case is registered by creating its directory; `build.rs` finds every
directory that holds a `test.rs` and generates the module declarations, so
